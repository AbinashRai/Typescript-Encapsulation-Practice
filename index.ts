class Bank {
  ...
  withdraw(amount: number) {
    if (amount > 0 && amount <= this.balance) {
      this.balance -= amount;
      console.log(
        `${amount} has been successfully withdrawn. New balance is ${this.balance}.`
      );
    } else {
      console.log(
        "The amount should be greater than zero and less than or equal to your balance"
      );
    }
  }
  ...
  deposit(amount: number) {
    if (amount > 0) {
      this.balance += amount;
      console.log(
        `${amount} has been deposited to your account, the new balance is ${this.balance}`
      );
    } else {
